...
细心的朋友会发现，在（1）中改变了a的值，但是b的值没变；但是在（2）中就不同了，对a进行的某种操作也会在b那里反映出来。但是，无论是哪种情况，在执行b=a之后，a和b表示的是**同一块数据**（指向内存中的同一片区域，即地址），而**不是相同数据的不同拷贝**，或者说，b是a的一个别名，你要找这片内存区域，说a也行，说b也行。
...
```ruby
if rand(100) < [0,100,80,60,40,20,0][self.state_ranks[j]]
```
...
```ruby
if skill_id == 1 and $game_party.item_number(2) < 3
  return false
end
```
...
```ruby
if self.is_a?(Game_Actor) && $data_weapons[self.weapon_id].element_set.include?(1) && skill.element_set.include?(1)
...
ryan = B.new
ryan.is_a?(A) #true 这里B类的实例当然属于A类
ryan.is_a?(B) #true
ryan.is_a?(C) #false ryan不是C类的实例
```
...
```ruby
$equipment_skill_table = {} #在这里定义的是Hash表
$equipment_skill_table[[0,1]] = [1,2] #这里定义1号武器的附带技能为1号和2号技能
# 注意，Hash表的主键是一个二元数组，第一个量表示装备种类，第二个量表示装备的ID，这样定义具有和系统的一致性。
```
...
if id == 0 or $game_party.weapon_number(id) > 0
  ...
  # 去掉被卸下装备的附带技能
# 对应技能设定的情况下
  if $equipment_skill_table.include?([0,@weapon_id])
for skill_id in $equipment_skill_table[[0,id]]
  ...
  # 增加新装备上的技能
  # 对应技能设定的情况下
  if $equipment_skill_table.include?([0,id])
for skill_id in $equipment_skill_table[[0,id]]
...
```ruby
def base_str
  return @enemy_id == 1 ? 1500 :$data_enemies[@enemy_id].str
end
```
